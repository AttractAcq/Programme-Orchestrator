...
After a run fails, readiness recomputation may leave its stage `ready` while the stage retains the failed run as `activeRunId` together with the same branch and worktree. An ordinary `ready` stage still cannot transition directly to `verifying`. `resume-run <runId> --from verification --by <actor>` uses a recovery-only transition for the narrower meaning “failed run with preserved implementation → resumed pre-commit verification”; it is available only when the failed run and all three retained stage links match.

Before mutating stage/run state or appending a recovery attempt/event, recovery confirms no builder/verifier process or other active run owns that worktree or branch, validates the Git top level and common repository, requires the stored branch at `HEAD`, proves descent from the stored base, rejects an existing result commit, and requires the intended dirty diff. Recovery never invokes the builder. Once those checks pass, it appends rather than replaces recovery and verification records, then reruns deterministic and independent verification. Only a pass permits the orchestrator commit and `awaiting_approval` transition. An invalid precondition leaves state and events unchanged; any later verification or commit failure returns the run to `failed` and preserves the worktree and earlier evidence.
